...
$order_field = isset($_GET['sfield']) && $_GET['sfield'] !=''?$_GET['sfield']:'create_date';
$order = isset($_GET['sorder']) && $_GET['sorder'] !=''?$_GET['sorder']:'desc';
$query
    ->select('*')
...
$sap = '&';
$burl = 'index.php?option=com_downloadmanager&view=packages';
$qr = '';
$paged = 1;
?>
...
<div class="w3eden">
    ...
    <table cellspacing="0" class="table table-hover table-striped">
        ...
        <tr>
            ...
            <th style="" class="manage-column column-media sortable <?php echo $jinput->get('sorder')=='asc'?'asc':'desc'; ?>" id="media" scope="col"><a href='<?php echo  $burl.$sap;?>sfield=title&sorder=<?php echo $jinput->get('sorder')=='asc'?'desc':'asc'; ?><?php echo $qr; ?>&paged=<?php echo $paged;?>'><span>Package Title</span> <?php if($jinput->get('sfield')=='title') { echo $jinput->get('sorder')=='asc'?'<i class="fa fa-angle-up" style="color:#D2322D;margin-left:10px"></i>':'<i class="fa fa-angle-down" style="color:#D2322D;margin-left:10px"></i>'; } ?></a></th>
            <th style="" class="manage-column column-media sortable <?php echo $jinput->get('sorder')=='asc'?'asc':'desc'; ?>" id="media" scope="col"><a href='<?php echo  $burl.$sap;?>sfield=category&sorder=<?php echo $jinput->get('sorder')=='asc'?'desc':'asc'; ?><?php echo $qr; ?>&paged=<?php echo $paged;?>'><span>Category</span> <?php if($jinput->get('sfield')=='category') { echo $jinput->get('sorder')=='asc'?'<i class="fa fa-angle-up" style="color:#D2322D;margin-left:10px"></i>':'<i class="fa fa-angle-down" style="color:#D2322D;margin-left:10px"></i>'; } ?></a></th>
            <th style="" class="manage-column column-media sortable <?php echo $jinput->get('sorder')=='asc'?'asc':'desc'; ?>" id="media" scope="col"><a href='<?php echo  $burl.$sap;?>sfield=create_date&sorder=<?php echo $jinput->get('sorder')=='asc'?'desc':'asc'; ?><?php echo $qr; ?>&paged=<?php echo $paged;?>'><span>Create Date</span> <?php if($jinput->get('sfield')=='create_date') { echo $jinput->get('sorder')=='asc'?'<i class="fa fa-angle-up" style="color:#D2322D;margin-left:10px"></i>':'<i class="fa fa-angle-down" style="color:#D2322D;margin-left:10px"></i>'; } ?></a></th>
		...
		<?php foreach($result as $post) {
            ...
            <tr valign="top" class="alternate author-self status-inherit" id="post-<?php echo $post->id; ?>">
                ...
                <td class="parent column-parent"><?php $catName= dm_get_package_category($post->category); echo'<a href="index.php?option=com_downloadmanager&view=category&cid='.$post->category.'">'.$catName.'</a>'; ?></td>
                <td class="parent column-parent"><?php echo date("M d, Y",$post->create_date); ?></td>
                ...
                <td class="actions"><a class="btn btn-primary btn-xs" href="index.php?option=com_downloadmanager&view=package&id=<?php echo $post->id; ?>"><i class="fa fa-pencil"></i></a>
                    <a target="_blank" class="btn btn-xs btn-inverse" href="<?php echo  str_replace('administrator/', '', JRoute::_(array('option' => 'com_downloadmanager', 'view' => 'package', 'id' => $post->id)));?>"><i class="fa fa-desktop"></i></a>
