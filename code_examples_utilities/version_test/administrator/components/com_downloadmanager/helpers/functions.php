...
define( 'JURL_DM_COM_ADMIN', JUri::root() . '/administrator/components/com_downloadmanager/' );
...
function dm_categories_checkboxes( $params ) {
	...
	if ( isset( $params['echo'] ) && $params['echo'] == 1 ) {
		echo $html;
	} else {
		return $html;
	}
...
function dm_get_package_category( $cid, $default = null ) {
	$db    = JFactory::getDBO();
	$query = $db->getQuery( true );
	$query
		->select( '*' )
	...
	return ! isset( $set->name ) || $set->name == '' ? $default : $set->name;
}